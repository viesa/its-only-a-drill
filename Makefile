...
INC_LOC 	:= -IC:\libarys\Chaos_SDL\include -Iinclude/audio -Iinclude/core -Iinclude/env -Iinclude/etc -Iinclude/file_io -Iinclude/game -Iinclude/net -Iinclude/UI
else
APP_C		:= app_client.out
APP_S		:= app_server.out
LIB_LOC 	:= -L/usr/lib/x86_64-linux-gnu
INC_LOC 	:= -Iinclude/audio -Iinclude/core -Iinclude/env -Iinclude/etc -Iinclude/file_io -Iinclude/game -Iinclude/net -Iinclude/UI
endif
...
# Source files
SRCDIR  	:= ./include
ALL_SRCS	:= $(wildcard $(SRCDIR)/*.c $(SRCDIR)/audio/*.c $(SRCDIR)/core/*.c  $(SRCDIR)/env/*.c $(SRCDIR)/etc/*.c $(SRCDIR)/file_io/*.c $(SRCDIR)/game/*.c $(SRCDIR)/net/*.c $(SRCDIR)/UI/*.c)
