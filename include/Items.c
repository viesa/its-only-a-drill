...
GroundListItems GroundListCreate(void)
{
    ...
    ground.contents[0] = ItemCreate(ItemWoodenSword);
    ground.contents[1] = ItemCreate(ItemMetalSword);
    ground.top = 2;
    return ground;
}
...
InventoryListItems InventoryCreate(void)
{
    ...
    i.contents[0] = ItemCreate(ItemWoodenSword); // to have the default case.
    i.top = 1;
    for (int tmp = 1; tmp < MAX_PLYER_ITEMS; tmp++)
    {
        i.contents[tmp] = ItemCreate(ItemEmpty);
    }
